<!-- Profile Header -->
<h1 align="center">👋 Hi, I'm <strong>Jayanth N</strong></h1>
<h3 align="center">Machine Learning • Deep Learning • Data Science</h3>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com/?lines=AI+%7C+Neural+Networks+%7C+Time+Series+Forecasting;Ocean+Data+Prediction+Researcher;Passionate+Learner+%26+Tech+Explorer&center=true&width=600&height=50">
</p>

---

### 🚀 About Me
- 🌊 Working on **CNN–GRU–Attention models for SST_C prediction**
- 🔬 Research experience in **oceanographic data forecasting**
- 📊 Love creating **complete ML pipelines — data → model → deployment**
- 📈 Interested in **NLP, Temporal Models & MLOps**
- 📍 Based in **India**

---

### 🛠️ Tech Stack
<p align="center">
  <img src="https://skillicons.dev/icons?i=python,tensorflow,pytorch,sklearn,java,mysql,git,github,docker,pandas,numpy,linux,opencv,html,css&perline=8" />
</p>

---

### 🌟 Featured Projects
🔹 **Marine Observation Data Retrieval & Visualization — INCOIS**  
CNN-GRU model to forecast **SST_C + dynamic dashboard (Looker Studio)**

🔹 **Multivariate Ocean Forecasting using Attention**  
Predicting **SST_C, SSS_psu & Chlorophyll** simultaneously using deep learning

🔹 **NLP Article Summarizer**  
TextRank + Transformers + Streamlit deployment

---

### 📊 GitHub Stats
<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=YOUR-USERNAME&show_icons=true&theme=tokyonight" height="150">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=YOUR-USERNAME&theme=tokyonight" height="150">
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR-USERNAME&layout=compact&theme=tokyonight" height="150">
</p>

---

### 🤝 Connect With Me
<p align="left">
  <a href="mailto:jayanth.work.email@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white"></a>
  <a href="https://linkedin.com/in/YOUR-LINKEDIN"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"></a>
  <a href="https://github.com/YOUR-USERNAME"><img src="https://img.shields.io/badge/GitHub-000000?style=for-the-badge&logo=github&logoColor=white"></a>
  <!-- Add portfolio or Kaggle if available -->
</p>

---

### ✨ Fun Fact
> *"AI will not replace humans — humans using AI will replace humans not using AI."*

---

<p align="center">
  ⭐ If you like my work, consider giving a star to my repositories!
</p>
